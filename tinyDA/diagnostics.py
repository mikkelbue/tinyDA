...
def plot_samples(samples, indices=[0, 1], plot_type="trace"):
    """Plot either traceplots or histograms of MCMC samples. The input must be
    a nxd array or list of nxd arrays, where n is the number of samples and d
    is the parameter dimension. Legacy function.
    ...
    warnings.warn(
        " plot_samples() has been deprecated. Please use to_inference_data() and ArviZ for posterior analysis."
    )

    samples = np.vstack(
        [samples["chain_{}".format(i)] for i in range(samples["n_chains"])]
    )
...
def plot_sample_matrix(samples, indices=[0, 1]):
    ...
    """Plot a pairs-plot with scatter and kde. The input must be a nxd array or
    list of nxd arrays, where n is the number of samples  and d is the
    ...
    warnings.warn(
        " plot_sample_matrix() has been deprecated. Please use to_inference_data() and ArviZ for posterior analysis."
    )
    ...
    g = g.map_upper(
        sns.scatterplot, size=1.0, color="#19068c", edgecolor="k", alpha=0.1
    )
...
def compute_R_hat(samples, rank_normalised=False, ess_auxiliary=False):
    ...
    """R-hat according to Vehtari et al. (2020). The first argument
    (parameters) must be list of arrays, one from each chain. Each array must
    be nxd, where n is the number of samples and d is the parameter dimension.
    Legacy function.
    ...
    warnings.warn(
        " compute_R_hat() has been deprecated. Please use to_inference_data() and ArviZ for posterior analysis."
    )
...
def compute_ESS(samples, rank_normalised=False):
    ...
    must be list of arrays, one from each chain. Each array must be nxd, where
    n is the number of samples and d is the parameter dimension. Legacy
    function.
    ...
    warnings.warn(
        " compute_ESS() has been deprecated. Please use to_inference_data() and ArviZ for posterior analysis."
    )
    ...
    for i in range(n_par):
        ...
        rho_t = (
            1
            - (W[i] - 1 / M * (s_m_sq[:, i][..., np.newaxis] * rho_t_m).sum(axis=0))
            / var_hat[i]
        )
...
def get_autocorrelation(x, T=None):
    ...
    warnings.warn(
        " get_autocorrelation() has been deprecated. Please use to_inference_data() and ArviZ for posterior analysis."
    )
...
def rank_normalise(samples):
    ...
    warnings.warn(
        " rank_normalise() has been deprecated. Please use to_inference_data() and ArviZ for posterior analysis."
    )
...
def compute_ESS_wolff(x):
    ...
    warnings.warn(
        " compute_ESS_wolff() has been deprecated. Please use to_inference_data() and ArviZ for posterior analysis."
    )
